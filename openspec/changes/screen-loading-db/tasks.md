...
- [ ] 3.1 Wire `ui.CorePool = dbPool.CorePool` in `cmd/golemui/main.go:54` (next to BusinessPool assignment)
- [ ] 3.2 Replace hardcoded `homeNode` (lines 61-74) with `LoadScreen(ctx, ui.CorePool, vistaID)` where vistaID defaults to `"home"` if `config.EntryPointViewID` is empty
- [ ] 3.3 Extend `TestRunBootstrap_Success` in `cmd/golemui/main_test.go` — register vista query in mock CorePool, assert `ui.CorePool` wired, assert window content composed from LoadScreen
- [ ] 3.4 Add `TestRunBootstrap_LoadScreenFailure` — mock returns `pgx.ErrNoRows`, assert pool closed, error returned
- [ ] 3.5 Run `go test ./...` — full suite passes
